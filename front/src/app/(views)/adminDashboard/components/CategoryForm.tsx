...
const CategoryForm = ({ categoryId }: CategoryFormProps) => {
  ...
  // Cargar categoría si estamos en edición
  useEffect(() => {
  ...
  const fetchCategory = async () => {
    try {
      setLoading(true);
      // TypeScript sabe que categoryId es string aquí
      ...
      <div className="w-full max-w-2xl p-8 rounded-lg shadow-xl bg-secondary-background-700">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5" autoComplete="off">
          ...
          <Button type="submit" disabled={loading} variant="dailyMenu">
            {loading ? (categoryId ? "Actualizando..." : "Creando...") : (categoryId ? "Actualizar categoría" : "Crear categoría")}
          </Button>

          {success && (
            <p className="flex items-center justify-center gap-2 mt-2 text-center text-green-500">
              <span className="text-xl">✅</span> {categoryId ? "Categoría actualizada con éxito" : "Categoría creada con éxito"}
            </p>
          )}
        </form>
      </div>
