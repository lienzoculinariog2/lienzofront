...
const RenderBarWithCart: React.FC<RenderBarProps> = ({ title }) => {
  ...
  const [priceRange, setPriceRange] = useState<{ min?: number; max?: number }>({ min: undefined, max: undefined });
  const [categories, setCategories] = useState<CategoryOption[]>([]);
...
const handleSearch = useCallback((category: CategoryOption, term: string, min?: number, max?: number) => {
    setSelectedCategory(category);
    setSearchTerm(term);
